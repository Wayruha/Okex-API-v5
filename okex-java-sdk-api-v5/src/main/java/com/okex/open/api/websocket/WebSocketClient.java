...
public class WebSocketClient<T> implements WebSocket {
    ...
    @Override
    public void subscribe(WSChannel... args) {
        String channels = formatArgs(Arrays.stream(args).map(JSONObject::toJSONString).toArray(String[]::new));
        this.send("{\"op\":\"subscribe\", \"args\":[" + channels + "]}");
    }

    @Override
    public void unSubscribe(WSChannel... args) {
        String channels = formatArgs(Arrays.stream(args).map(JSONObject::toJSONString).toArray(String[]::new));
